...
namespace CleanArchitecture.Application.Features.AuthFeatures.Commands.Login
{
    public sealed class LoginCommandValidator : AbstractValidator<LoginCommand>
    {
        ...
        {
            RuleFor(p => p.UserNameOrEmail).NotEmpty().WithMessage("Kullanıcı adı ya da email boş olamaz!");
            RuleFor(p => p.UserNameOrEmail).NotNull().WithMessage("Kullanıcı adı ya da email boş olamaz!");
            RuleFor(p => p.UserNameOrEmail).MinimumLength(3).WithMessage("Kullanıcı adı ya da email en az 3 karakter olmalıdır!");
